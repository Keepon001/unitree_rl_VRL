...
void State_RL::_observations_compute()
{
    ...
    torch::Tensor commands = torch::cat({torch::tensor({_vCmdBody(0)}), torch::tensor({_vCmdBody(1)}), torch::tensor({_dYawCmd}),
                                         torch::tensor({_bodyHeightCmd}), frequencies, torch::tensor({_phases}),
                                         torch::tensor({_offsets}), torch::tensor({_bounds}), torch::tensor({_durations})}, -1);
    
    // 1. 更新步态相位 (_gait_indices), 假设 _gait_indices 初 始为0
    _gait_indices = torch::remainder(_gait_indices + _RL_dt * frequencies, 1.0);
    // 2. 为每个脚步更新相位（假设四个脚或动作）
    _foot_indices[0][0] = _gait_indices[0][0] + _phases + _offsets + _bounds;  
    _foot_indices[0][1] = _gait_indices[0][0] + _offsets;
    _foot_indices[0][2] = _gait_indices[0][0] + _bounds;
    _foot_indices[0][3] = _gait_indices[0][0] + _phases;
    
    // 3. 更新时钟输入 (_clock_inputs)，通常用正弦/余弦表示周期性动作
    // 使用正弦和余弦函数将相位映射为周期性信号
    for (int i = 0; i < 4; ++i) {
        _foot_indices[0][i] = torch::remainder(_foot_indices[0][i], 1.0);
        _clock_inputs[0][i] = torch::sin(2 * M_PI * _foot_indices[0][i]);  // 正弦信号
        //_clock_inputs[0][i] = torch::cos(2 * M_PI * _foot_indices[0][i]);  // 或者余弦信号
    }
    std::cout<<"foot_indices: "<<_clock_inputs[0][0]<<"   "<<_clock_inputs[0][1]<<"   "<<_clock_inputs[0][2]<<"   "<<_clock_inputs[0][3]<<std::endl;
    // 相对关节角度
    ...
    // 3 + 9 + 12 + 12 + 12 + 12 + 4
    this->_observation = torch::cat({projected_gravity.view({1, -1}),
                                ...
                                _action.view({1, -1}),
                                _last_action.view({1, -1}),
                                _clock_inputs.view({1, -1}),
                                }, -1);
    ...
    std::cout << "Observation tensor: " << this->_observation << std::endl;
...
void State_RL::_getUserCmd(){
    ...
    _dYawCmd = 0 * _dYawCmdPast + (1-0) * _dYawCmd;
    _dYawCmdPast = _dYawCmd;
    _bodyHeightCmd = 0.05 * _userValue.ry;
    //std::cout<<_vCmdBody(0)<< ""<<_vCmdBody(1)<<""<< _dYawCmd <<std::endl;
#else
    std::cout << "Xbox joystick support is disabled." << std::endl;
    /* Movement */
    _vCmdBody(0) =  invNormalize(_userValue.ly, _vxLim(0), _vxLim(1));
    _vCmdBody(1) = -invNormalize(_userValue.lx, _vyLim(0), _vyLim(1));
    _vCmdBody(2) = 0;
...
void State_RL::_obs_buffer_update()
{
    ...
    // 打印更新后的观测历史缓冲区（可选，用于调试）
    //std::cout << this->_obs_buffer_tensor << std::endl;
}
...
void State_RL::_action_compute()
{
    ...
    this->_action = _body_module.forward({torch::cat({this->_obs_buffer_tensor, latent}, -1)}).toTensor();
