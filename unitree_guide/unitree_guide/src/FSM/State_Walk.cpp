...
State_Walk::State_Walk(CtrlComponents *ctrlComp)
    : FSMState(ctrlComp, FSMStateName::WALK, "rl"), 
...
void State_Walk::_loadPolicy()  // 加载JIT模型
{
    this->_actor_module = torch::jit::load(model_path);
}

torch::Tensor State_Walk::QuatRotateInverse(torch::Tensor q, torch::Tensor v)
{
...
void State_Walk::_observations_compute()
{
    // 获取当前的观测信息
...
void State_Walk::_getUserCmd(){
    ...
    /* Movement */
    _vCmdBody(0) = 8 * invNormalize(_userValue.ly, _vxLim(0), _vxLim(1));
    _vCmdBody(1) = -2.5 * invNormalize(_userValue.lx, _vyLim(0), _vyLim(1));
    _vCmdBody(2) = 0;

    /* Turning */
    _dYawCmd = -5 * invNormalize(_userValue.rx, _wyawLim(0), _wyawLim(1));
...
void State_Walk::_obs_buffer_update()
{
    // 更新输入的历史观测缓冲区
    ...
    // 从当前的观测历史缓冲区中移除最旧的 _num_obs 个元素
    // 并保留前面的所有历史数据
    auto narrow_tensor = this->_obs_buffer_tensor.narrow(1, 0, this->_num_obs_history * this->_num_obs - this->_num_obs);

    // 将当前观测数据转换为一个与 _obs_buffer_tensor 兼容的形状
    auto new_obs_tensor = this->_observation.view({1, this->_num_obs});

    // 将新的观测数据追加到缓冲区的开头
    this->_obs_buffer_tensor = torch::cat({new_obs_tensor, narrow_tensor}, -1);

    // 打印更新后的观测历史缓冲区（可选，用于调试）
    //std::cout << this->_obs_buffer_tensor << std::endl;
}

void State_Walk::_action_compute()
{
    // 以50hz调用rl_policy来生成关节角度
    torch::NoGradGuard no_grad;
    this->_action = _actor_module.forward({this->_obs_buffer_tensor}).toTensor();
...
void State_Walk::enter()
{
    ...
    if (!_inferenceThread.joinable()) {  // joinable检查线程是否在执行中，执行中会返回true。
        _inferenceThread = std::thread(&State_Walk::_inferenceLoop, this);  // 创建线程
    }
}

void State_Walk::_inferenceLoop() {  // 模型推理线程
...
FSMStateName State_Walk::checkChange()
{
