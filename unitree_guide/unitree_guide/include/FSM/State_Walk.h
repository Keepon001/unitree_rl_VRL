...
{
public:
    State_Walk(CtrlComponents *ctrlComp);
    ~State_Walk() {}
    void enter();
    void run();
    void exit();
...
private:
    const std::string model_path = "/home/chuixue/roswork/unitree_rl/src/unitree_guide/unitree_guide/model/WALK/policy.pt";
    ...
    torch::jit::script::Module _actor_module;

    // 可能需要调整的参数
    const float Kp = 40.0;   //宇树原版60.0;  改成模型对应
    const float Kd = 1.0;   //5.0;
    const int _num_obs = 45;  // 观测数量
    const int _num_obs_history = 1;  // 历史观测时间步
